...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        self.W = tf.Variable(tf.zeros(shape=(self.config.n_features, self.config.n_classes), dtype=tf.float32), name="weights")
        self.b = tf.Variable(tf.zeros(shape=(self.config.n_classes,), dtype=tf.float32), name="biases")
